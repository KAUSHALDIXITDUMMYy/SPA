...
/**
 * Agora Stream Cleanup Script - Direct API Version (Simple)
 * 
 * This script directly checks Agora API for active channels and users,
 * then force disconnects any lingering connections using only APP_ID and APP_CERTIFICATE.
 */
...
class AgoraDirectCleanup {
  ...
  /**
   * Generate Agora API authentication header using APP_ID and APP_CERTIFICATE
   */
  generateAuthHeader() {
  ...
  async getActiveChannels() {
    ...
    try {
      ...
      const response = await fetch(`${AGORA_BASE_URL}/dev/v1/channel/${AGORA_APP_ID}`, {
        method: 'GET',
        headers: {
  ...
  async getChannelUsers(channelName) {
    try {
      const authHeader = this.generateAuthHeader();
      const response = await fetch(`${AGORA_BASE_URL}/dev/v1/channel/user/${AGORA_APP_ID}/${encodeURIComponent(channelName)}`, {
        method: 'GET',
        headers: {
          'Authorization': authHeader,
          'Content-Type': 'application/json'
  ...
  async kickUserFromChannel(channelName, userId) {
    try {
      const authHeader = this.generateAuthHeader();
      const response = await fetch(`${AGORA_BASE_URL}/dev/v1/kick/${AGORA_APP_ID}/${encodeURIComponent(channelName)}/${userId}`, {
        method: 'POST',
        headers: {
          'Authorization': authHeader,
          'Content-Type': 'application/json'
  ...
  async checkForLingeringChannels() {
    ...
    if (lingeringChannels.length > 0) {
      ...
      for (const channel of lingeringChannels) {
        ...
        const usersData = await this.getChannelUsers(channel.channelName);
        if (usersData.users && usersData.users.length > 0) {
          this.activeUsers.push(...usersData.users.map(userId => ({
            uid: userId,
