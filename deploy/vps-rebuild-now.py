#!/usr/bin/env python3
"""Clean rebuild on VPS after a failed partial build."""
import os
import sys
...
def main() -> int:
    ...
    log("=== clean rebuild ===")
    build_cmd = (
        f"cd {APP_DIR} && rm -rf .next node_modules && npm ci && npm run build && "
    ...
    out = run(client, build_cmd, timeout=900)
    log(out[-8000:])
    ...
    log("=== pm2 restart ===")
    log(run(client, f"cd {APP_DIR} && pm2 restart ecosystem.config.cjs --update-env && pm2 save"))

    log("=== verify commit ===")
    log(run(client, f"cd {APP_DIR} && git log -1 --oneline"))
